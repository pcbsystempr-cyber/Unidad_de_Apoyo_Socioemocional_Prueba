...
document.addEventListener('DOMContentLoaded', () => {
    // AI Bubble mouse following effect
    const aiBubble = document.getElementById('ai-bubble');
    if (aiBubble) {
        let mouseX = 0, mouseY = 0;
        
        document.addEventListener('mousemove', (e) => {
            mouseX = (e.clientX / window.innerWidth * 100 - 50) * 0.15; // Subtle: 15% intensity
            mouseY = (e.clientY / window.innerHeight * 100 - 50) * 0.15;
            
            aiBubble.style.setProperty('--mouse-x', `${mouseX}px`);
            aiBubble.style.setProperty('--mouse-y', `${mouseY}px`);
        });
    }
